...
router.get("/event", middleware, (req, res, next) => {
  ...
  con.query(
    "SELECT  DATE_FORMAT(ap_date_start,'%Y-%m-%d') AS ap_date_start   FROM app_appointment WHERE ap_learn_type  = ? AND DATE(ap_date_start) >= ? ORDER BY ap_date_start ASC LIMIT 0,30",
